(function() {
    var obj = function(env, pkg) {
        ...
        this.dockerPath = (serverName) => {
           return _env.data_folder + '/sites/' + serverName + '/docker/';;
        }

        this.dockerFileFn = (serverName) => {
            var sites_list = me.getSitesCfg();
            var site_config = sites_list[serverName];
           return me.dockerPath(serverName) + site_config.docker.dockerFile;
        }

        this.siteContainer = (serverName) => {
            return (serverName + '-container').toLowerCase();
        }

        this.siteImage = (serverName) => {
            return ('webserver-' + serverName + '-image').toLowerCase();
        }
        ...
        this.addDockerCMD = (serverName) => {
            ...
            var cmd = '';
            cmd += 'cd ' + me.dockerPath(serverName) + "\n";
            cmd += 'docker build -f ' + me.dockerFileFn(serverName) + ' -t ' + me.siteImage(serverName) + ' .' + "\n";
            cmd += me.removeDockerContainerCMD();
            var cmd_ports  = '';
            let ports = (!site_config || !site_config.docker) ? [] : site_config.docker.ports;
            for (var i = 0;  i < ports.length; i++) {
            ...
            cmd += 'docker run -d ' + cmd_ports + ' -v "'+ site_path + '":/var/_localApp  --network network_easydocker --name ' + me.siteContainer(serverName) + ' ' + me.siteImage(serverName) + "\n";
            
            return cmd;
        }

        this.buildDockerImageCMD = (serverName) => {
            var cmd = '';
            cmd += 'cd ' + me.dockerPath(serverName) + "\n";
            cmd += 'docker build -f ' + me.dockerFileFn(serverName) + ' -t ' + me.siteImage(serverName) + ' .' + "\n";
            return cmd;
        ...
        this.removeDockerImageCMD = (serverName) => {
            return 'docker image rm -f ' + serverName.toLowerCase() + '-image' + "\n";
        }
        ...
        this.removeDocker = (serverName, callback) => {
            me.setClone('removeDocker', me.removeDockerContainerCMD(serverName) + me.removeDockerImageCMD(serverName), callback);
        }
    }
    module.exports = obj;
})()
